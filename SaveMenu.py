...
def SaveMenu(screen, type, scene, line, image, characters):
    continuer = True
    saves = getSaves(1)
    page = 1
    cursor = 0
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                if event.key == K_DOWN and cursor < 4:
                    ...
                    # On sauvegarde ici
                    utils.save((page - 1) * 5 + cursor - 1, type, scene, line, image, characters)
                    # Puis on retourne à notre state
                    return [type, scene, line]

                elif event.key == K_ESCAPE or event.key == K_DELETE:
                    # On abandonne la sauvegarde, on retourne donc à notre stat
                    return [type, scene, line]
